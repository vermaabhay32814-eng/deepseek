...
const Sidebar = ({expand, setExpand}) => {
  ...
  return (
    <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'w-64 p-4' : 'md:w-20 w-0 max-md:overflow-hidden'} `}>
        <div>
            ...
            <button className={`mt-8 flex items-center justify-center cursor-pointer 
                ${expand ? "bg-primary hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max" : "group relative w-9 h-9 ml-6 max-auto hover:bg-gray-500/30 rounded-lg"}`}>
         ...
         <div>
            <div className={`flex items-center cursor-pointer relative group ${expand ? "gap-1 text-white/80 text-sm p-2.5 border-primary rounded-lg hover:bg-white/10 cursor-pointer" : "h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg" }`}>
